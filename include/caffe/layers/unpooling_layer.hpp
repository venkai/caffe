...
/**
 * @brief Unpools the input activation based on supplied pool mask.
 *        Pool mask is the second bottom blob, which is typically the output
 *        of some MAX/STOCHASTIC-pooling layer in the network.
...
class UnpoolingLayer : public Layer<Dtype> {
 public:
  ...
  // MAX UNPOOL layers can output an extra top blob for the mask;
