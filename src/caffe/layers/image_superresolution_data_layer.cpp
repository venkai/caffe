...
namespace caffe {
    ...
    template <typename Dtype>
    void ImageSuperResolutionDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
        ...
        CHECK( transform_param.has_ds_param() ) << "ImageSuperResolutionDataLayer requires ds_param";
        ...
        //label
        top[1]->Reshape(batch_size, label_channels, label_height, label_width);
        for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
            this->prefetch_[i].label_.Reshape(batch_size, label_channels, label_height, label_width);
        }
        this->transformed_label_.Reshape(1, label_channels, label_height, label_width);     
    ...
    void ImageSuperResolutionDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
        ...
        for (int item_id = 0; item_id < batch_size;) {
            ...
            if (this->layer_param_.transform_param().ds_param().skip_small() && (img_row < max_height || img_col < max_width)){
                lines_id_++;
                continue;
            }
