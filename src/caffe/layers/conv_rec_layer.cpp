...
namespace caffe {
    ...
    const vector<Blob<Dtype>*>& top) {
        ...
        switch (Caffe::mode()) {
          ...
          caffe_gpu_set(C_, Dtype(0), tau_.mutable_gpu_data());
        ...
        if (this->phase_ == TEST) {
            wt_inv_.ShareData(*(this->blobs_[0]));
        } else {
            switch (Caffe::mode()) {
              case Caffe::CPU:
              //caffe_copy(wt_inv_.count(), this->blobs_[0]->cpu_data(), wt_inv_.mutable_cpu_data());
              caffe_set(wt_inv_.count(), Dtype(0), wt_inv_.mutable_cpu_data());
              case Caffe::GPU:
#ifndef CPU_ONLY
              //caffe_copy(wt_inv_.count(), this->blobs_[0]->gpu_data(), wt_inv_.mutable_gpu_data());
              caffe_gpu_set(wt_inv_.count(), Dtype(0), wt_inv_.mutable_gpu_data());
#endif          
              break;
            }   
        }
    ...
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
        ...
        for (int iter = Nrec_-1; iter >=0; --iter) {
            ...
            caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, batch_size_, C_, C_, (Dtype)1., bottom[0]->cpu_data(), wt_inv_data + wt_offset, (Dtype)0., mid_.mutable_cpu_data());
