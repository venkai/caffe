...
void ImageSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // Read a label, and use it to obtain number of label channels.
  cv::Mat cv_seg_tmp = ReadImageToCVMat(root_folder + lines_[lines_id_].second,
                                    new_height, new_width, is_label_color);

  const int label_channels = cv_seg_tmp.channels();
  ...
  if (crop_size > 0) {
    ...
    for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
        this->prefetch_[i].label_.Reshape(batch_size, label_channels, crop_size, crop_size);
    }
...
void ImageSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    int img_row, img_col;
    cv_img_seg.push_back(ReadImageToCVMatDeconvnet(root_folder + lines_[lines_id_].first,
    ...
    if (label_type == ImageDataParameter_LabelType_PIXEL) {
      ...
      cv_img_seg.push_back(ReadImageToCVMatDeconvnet(root_folder + lines_[lines_id_].second,
              new_height, new_width, is_label_color, &img_row, &img_col));
