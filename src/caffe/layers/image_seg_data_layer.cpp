...
void ImageSegDataLayer<Dtype>::DataLayerSetUp(
  ...
  if (crop_size > 0) {
    // data
    const vector<int> data_shape{batch_size, channels, crop_size, crop_size};
    top[0]->Reshape(data_shape);
    for (int i = 0; i < this->prefetch_.size(); ++i) {
        this->prefetch_[i]->data_.Reshape(data_shape);
    }
    const vector<int> one_data_shape{1, channels, crop_size, crop_size};
    this->transformed_data_.Reshape(one_data_shape);
    // label
    const vector<int>
        label_shape{batch_size, label_channels, crop_size, crop_size};
    top[1]->Reshape(label_shape);
    for (int i = 0; i < this->prefetch_.size(); ++i) {
        this->prefetch_[i]->label_.Reshape(label_shape);
    }
    const vector<int> one_label_shape{1, label_channels, crop_size, crop_size};
  ...
  const vector<int> prefetch_dim_shape{batch_size, 1, 1, 2};
  top[2]->Reshape(prefetch_dim_shape);
  this->prefetch_data_dim_.Reshape(prefetch_dim_shape);
  // data
