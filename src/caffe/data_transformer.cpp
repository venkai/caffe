...
/** Venkat (6-Oct-2016): 
taken from https://github.com/ducha-aiki/caffe/blob/augmentations/src/caffe/data_transformer.cpp **/
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformInputAndLabel(const std::vector<cv::Mat>& cv_input_label,
Blob<Dtype>* transformed_data_blob, Blob<Dtype>* transformed_label_blob) {
    ...
    CHECK(cv_input_label[0].depth() == CV_8U) << "Image data type must be unsigned byte";
    CHECK(cv_input_label[1].depth() == CV_8U) << "Label data type must be unsigned byte";
...
/*** Venkat: Insert Other Useful Image-to-Image Regression Transformations here--  ***/
    ...
    if ( param_.has_super_res_param() ){
        ...
        CHECK(min_sr>=1 && max_sr>=min_sr)<<"max_sr_scale>=min_sr_scale>=1.";
        CHECK((param_.super_res_param().has_interp_mode() &&
        !param_.super_res_param().has_down_interp_mode() && !param_.super_res_param().has_up_interp_mode()) ||
        ...
        <<"\nOR separately specify down_interp_mode/up_interp_mode (without specifying interp_mode).";
        ...
        if (max_sr==min_sr)
            ...
            //Generate a scaling factor "rand_sr" uniformly randomly between min_sr and max_sr
            caffe_rng_uniform(1, min_sr, max_sr, &rand_sr);
        }
        if (rand_sr>1.0) {
            int down_interp_mode,up_interp_mode;
            if (param_.super_res_param().has_interp_mode()){
                down_interp_mode=up_interp_mode=sr_to_cv_interp(param_.super_res_param().interp_mode());
            }
            else {
                down_interp_mode=sr_to_cv_interp(param_.super_res_param().down_interp_mode());
                up_interp_mode=sr_to_cv_interp(param_.super_res_param().up_interp_mode());
            }
            CHECK(down_interp_mode!=-1)<<"Invalid Interpolation for downsampling";
            CHECK(up_interp_mode!=-1)<<"Invalid Interpolation for upsampling";
            cv::Mat cv_resized_img;
            //Downscale resized image by factor of "rand_sr" using desired down-interpolation method
            cv::resize(cv_cropped_img,cv_resized_img,cv::Size(),(1.0/rand_sr),(1.0/rand_sr),down_interp_mode);
            ...
            cv::resize(cv_resized_img,cv_cropped_img,cv::Size(cv_cropped_img.cols,cv_cropped_img.rows),0,0,up_interp_mode);
        }
    }
    ...
    if ( num_wgn_policies > 0 ){
        ...
        CHECK((wgn_param.has_noise_std() && !wgn_param.has_min_noise_std() && !wgn_param.has_max_noise_std()) ||
        (!wgn_param.has_noise_std() && wgn_param.has_min_noise_std() && wgn_param.has_max_noise_std()) ||
        (!wgn_param.has_noise_std() && !wgn_param.has_min_noise_std() && !wgn_param.has_max_noise_std()))
        <<"\n\nSpecify EITHER no noise std (defaults to 25), OR specify only noise_std,"
        <<"\nOR separately specify min_noise_std/max_noise_std (without specifying noise_std).";
        float noise_std=0.0;
        if (wgn_param.has_noise_std()||(!wgn_param.has_min_noise_std() && !wgn_param.has_max_noise_std())) {
            ...
            caffe_rng_uniform(1, wgn_param.min_noise_std(), wgn_param.max_noise_std(), &noise_std);
        }
    ...
    if ( param_.has_jpeg_param() ){
        ...
        CHECK(min_quality>=1 && max_quality>=min_quality)<<"max_quality>=min_quality>=1.";
        float rand_quality=20.0;
        if (max_quality==min_quality){
            rand_quality=max_quality;
        } else {
            //Generate a scaling factor "rand_quality" uniformly randomly between min_quality and max_quality
...
Blob<Dtype>* transformed_data_blob, Blob<Dtype>* transformed_label_blob) {
    ...
    CHECK(cv_img_full.depth() == CV_8U) << "Image data type must be unsigned byte";
    ...
    cv::Mat cv_resized_img;
    const int down_interp_mode=sr_to_cv_interp(param_.ds_param().interp_mode());
    //Downscale resized image by factor of "ds_scale" using desired down-interpolation method
    //cv::resize(cv_img,cv_resized_img,cv::Size(),(1.0/ds_scale),(1.0/ds_scale),down_interp_mode);
    cv::resize(cv_img,cv_resized_img,cv::Size((cv_img.cols)/ds_scale,(cv_img.rows)/ds_scale),0,0,down_interp_mode);
    //Convert to [0,255] range integer type for realistic evaluation
    cv_resized_img.convertTo(cv_resized_img,CV_8U);
    //Convert back to double
    cv_resized_img.convertTo(cv_resized_img,CV_64F);
    ...
    for (int h = 0; h < label_height; ++h) {
        ...
        for (int w = 0; w < label_width; ++w) {
            ...
            for (int c = 0; c < label_channels; ++c) {
                if (do_mirror) {
                    top_index = (c * label_height + h) * label_width + (label_width - 1 - w);
                } else {
